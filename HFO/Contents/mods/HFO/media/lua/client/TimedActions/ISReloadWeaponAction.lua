...
-------------------------------------------------------------------------
-- ADAPTING JAM CHANCE CALCULATION TO BE INFLUENCED BY SANDBOX OPTIONS --
-------------------------------------------------------------------------
HFO = HFO or {};
local sVars = SandboxVars.HFO;
sVars.JamChance = sVars.JamChance or 4;

-- shoot shoot bang bang
-- handle ammo removal, new chamber & jam chance
ISReloadWeaponAction.onShoot = function(player, weapon)
	local jamChance = 4
	if sVars.JamChance == 1 then
		jamChance = 24
	elseif sVars.JamChance == 2 then
		jamChance = 12
	elseif sVars.JamChance == 3 then
		jamChance = 8
	elseif sVars.JamChance == 4 then
		jamChance = 4
	else
		jamChance = 2
	end
	...
	if weapon:getCondition() < weapon:getConditionMax() and weapon:getJamGunChance() > 0 and weapon:getCurrentAmmoCount() > 0 then
		...
		baseJamChance = baseJamChance + ((weapon:getConditionMax() - weapon:getCondition()) / jamChance)
